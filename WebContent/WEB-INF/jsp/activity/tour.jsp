...
<body>
	...
	<%@include file="/menu/bpm-workspace.jsp"%>
		...
		<header class="header">
		<h4 class="title">自驾游</h4>
		</header>
		
		<div class="content">
		<c:choose>
			<c:when test="${empty requestScope.page.list }">
			<center><h5>暂无自驾游</h5></center>
			</c:when>
			<c:otherwise>
				<br>
		...
		<div class="m-pagination pull-right">
			<a href="${pageContext.request.contextPath}/activity/tour/1">首页</a>
			<c:if test="${requestScope.page.currentPage != 1 }">
				<a href="${pageContext.request.contextPath}/activity/tour/${requestScope.page.currentPage -1}">上一页</a>
			</c:if>
			<c:if test="${requestScope.page.currentPage != requestScope.page.pageCount }">
				<a href="${pageContext.request.contextPath}/activity/tour/${requestScope.page.currentPage +1}">下一页</a>
			</c:if>
			<a href="${pageContext.request.contextPath}/activity/tour/${requestScope.page.pageCount }">尾页</a>
			当前第
